...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5581983678841@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5581983678841@s.whatsapp.net'
                        /******ApiKey Input******/
               ...
               break
				case 'rebaixar':
                  ...
                  case 'promover':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Calma esse membro mamou muito a administração e ganhou o ADM :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 FLAVIO BOT 〙', members_id, true)
					break
                case 'marcar2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 FLAVIO BOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'marcar4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
